...
以下示例展示如何使用 `pku3b_py` 模块结构化访问课程内容并下载附件。

### 🔗 初始化客户端并获取课程列表

```python
from pku3b_py import Client
...
for doc in course.documents():
    print("标题：", doc.title())
    print("正文段落：", doc.descriptions())
    doc.download("./downloads")  # 附件将自动识别后缀保存
```
...
for ann in course.announcements():
    print("通知标题：", ann.title())
    print("正文段落：", ann.descriptions())
    ann.download("./announcements")  # 图片附件将保存为 jpg/png 格式
```
...
for assn in course.assignments():
    print("作业标题：", assn.title())
    print("作业内容：", assn.descriptions())
    assn.download("./assignments")  # 作业附件支持批量下载
```
...
for vid in course.videos():
    print("回放标题：", vid.title())
    print("视频说明：", vid.descriptions())
    vid.download("./videos")  # 下载为 mp4 格式，支持断点续传
```

---
### ✅ 功能特性

- 所有 `.download(path)` 调用都支持自动识别文件后缀
- 所有 `.descriptions()` 都返回结构化段落数组
- 所有内容都统一封装为 CourseContentData，便于递归整合与 AI 使用
